...
use gitstratum_proto::{
    ...
    GetCommitRequest, GetRefRequest, GetTreeRequest, ListRefsRequest, ListReposRequest,
    Oid as ProtoOid, PutCommitRequest, PutTreeRequest, Signature as ProtoSignature,
...
#[tokio::test]
async fn test_repository_grpc_lifecycle() {
    ...
    assert_eq!(response.repo_ids.len(), 4);
...
#[tokio::test]
async fn test_ref_grpc_operations() {
    let (service, _temp) = create_test_service();
    ...
    let response = service.get_ref(get_request).await.unwrap().into_inner();
    assert!(response.found);
    assert_eq!(response.target.unwrap().bytes, oid1.bytes);
    ...
    for name in &["refs/heads/feature", "refs/tags/v1.0"] {
...
#[tokio::test]
async fn test_commit_and_tree_grpc_storage() {
    let (service, _temp) = create_test_service();
    ...
    let response = service.put_tree(put_request).await.unwrap().into_inner();
    assert!(response.success);
    ...
    let response = service.get_tree(get_request).await.unwrap().into_inner();
    assert!(response.found);
    let fetched = response.tree.unwrap();
    assert_eq!(fetched.oid, tree.oid);
    assert_eq!(fetched.entries.len(), 1);

    let tree_oid = create_proto_oid(b"tree");
    let commit = create_proto_commit(&tree_oid, vec![], "Test commit");

    let put_request = Request::new(PutCommitRequest {
        repo_id: "test-repo".to_string(),
        commit: Some(commit.clone()),
    });
    let response = service.put_commit(put_request).await.unwrap().into_inner();
    assert!(response.success);
    ...
    let response = service.get_commit(get_request).await.unwrap().into_inner();
    assert!(response.found);
    let fetched = response.commit.unwrap();
    assert_eq!(fetched.oid, commit.oid);
    assert_eq!(fetched.message, "Test commit");

    let nonexistent = create_proto_oid(b"nonexistent");
    let request = Request::new(GetCommitRequest {
        repo_id: "test-repo".to_string(),
        oid: Some(nonexistent),
    });
...
#[tokio::test]
async fn test_graph_grpc_operations() {
    let (service, _temp) = create_test_service();
